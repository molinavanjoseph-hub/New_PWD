...
export const createBusinessJobPost = async (payload = {}) => {
  ...
  try {
    ...
    if (!createdJob) {
      throw new Error('The Firebase function saved the job post, but no job data was returned.')
    }
    ...
    if (functionCode === 'functions/internal' || functionCode === 'internal') {
      throw new Error('The job post could not be saved right now. Check the deployed function logs, then try again.')
    }

    if (functionCode !== 'functions/not-found') {
      throw new Error(
        toJobsErrorMessage(
          functionError,
