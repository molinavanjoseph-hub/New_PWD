...
const heroImageStyle = layerStyle(0.18)
const heroContentStyle = computed(() => ({
  transform: `translate3d(0, ${scrollY.value * 0.1}px, 0)`,
}))
const heroLogoStyle = computed(() => ({
  transform: `translate3d(0, calc(-1.2rem + ${scrollY.value * -0.04}px), 0) scale(${1 + Math.min(scrollY.value, 120) * 0.00005})`,
}))
const heroAccentStyle = computed(() => ({
  transform: `translate3d(0, ${scrollY.value * 0.14}px, 0)`,
}))
...
const applyFeaturedJobs = (rows = []) => {
  featuredJobPosts.value = Array.isArray(rows)
    ? rows
  ...
  if (selectedFeaturedJobIndex.value >= featuredJobs.value.length) {
    selectedFeaturedJobIndex.value = 0
  }
}

const startFeaturedJobsSync = () => {
  stopPublicJobsSync()
  isFeaturedJobsLoading.value = true
  stopPublicJobsSync = subscribeToPublicJobs(
    (rows) => {
      applyFeaturedJobs(rows)
    },
    () => {
      applyFeaturedJobs([])
    },
...
const resolveInitialLoading = (delay = 0) => {
  if (hasResolvedInitialLoading) return
...
<template>
  <!-- Landing page shell -->
  <div
    class="relative flex min-h-screen flex-col overflow-x-clip text-[#f4f1e8] transition-colors duration-500 ease-out"
    :class="isPageLoading ? 'bg-[#061810]' : 'bg-[#f5f4ef]'"
  >
    <!-- First loading screen while the page is preparing -->
    ...
    <transition name="loading-overlay">
      <div v-if="isHeroSearchLoading" class="loading-screen" role="status" aria-live="polite" aria-label="Searching jobs">
        <div class="loading-screen__aurora loading-screen__aurora--one" />
        <div class="loading-screen__aurora loading-screen__aurora--two" />
        <div class="loading-screen__grid" />
        <div class="loading-screen__panel">
          ...
          <img class="loading-screen__wordmark" :src="pwdWordmark" alt="PWD Platform" decoding="async" fetchpriority="high" />
          <p class="hero-search-loading-title">Searching jobs for you</p>
          <p class="hero-search-loading-subtitle">Please wait while we prepare the search results.</p>
          <div class="loading-screen__dots" aria-hidden="true">
            <span class="loading-screen__dot" />
            <span class="loading-screen__dot" />
            <span class="loading-screen__dot" />
    ...
    <transition name="loading-overlay">
      <div v-if="isLoginRouteLoading" class="loading-screen" role="status" aria-live="polite" aria-label="Opening login page">
        <div class="loading-screen__aurora loading-screen__aurora--one" />
        <div class="loading-screen__aurora loading-screen__aurora--two" />
        <div class="loading-screen__grid" />
        <div class="loading-screen__panel">
          ...
          <img class="loading-screen__wordmark" :src="pwdWordmark" alt="PWD Platform" decoding="async" fetchpriority="high" />
          <p class="hero-search-loading-subtitle">Please wait while we prepare the sign in screen.</p>
          <div class="loading-screen__dots" aria-hidden="true">
            <span class="loading-screen__dot" />
            <span class="loading-screen__dot" />
            <span class="loading-screen__dot" />
    ...
    <!-- Main page content -->
    <div
      class="flex flex-1 flex-col transition-opacity duration-700 ease-out"
      :class="isPageLoading ? 'opacity-0' : 'opacity-100'"
    >
    <!-- Top navigation -->
    <header
      class="landing-rise-in relative z-10 border-b border-[#dce6e0] shadow-[0_10px_24px_rgba(23,35,29,0.06)] transition-[padding,transform] duration-300"
      :class="
        isNavScrolled
          ? 'bg-white pt-3'
          : 'bg-white pt-5'
    ...
    <main class="relative z-[1] mx-auto flex flex-1 flex-col max-w-[1180px] px-4 pb-0 pt-12 sm:px-[clamp(1rem,3vw,2rem)] max-[640px]:px-[0.8rem]">
      ...
      >
        ...
        >
          <div
            class="landing-hero__copy landing-rise-in relative z-[1] -mt-20 w-full max-w-[820px] overflow-visible self-start will-change-transform max-[900px]:-mt-8"
          ...
          >
            ...
            <div class="hero-search-shell">
              <form class="hero-search" @submit.prevent="submitHeroSearch">
                ...
                <label class="hero-search__field hero-search__field--select">
                  <div class="hero-search__control hero-search__control--select">
                    <select v-model="heroSearchJobCategory" class="hero-search__select" aria-label="All Categories">
                      <option value="">All Categories</option>
                      <option v-for="option in heroJobCategoryOptions" :key="option" :value="option">{{ option }}</option>
                    </select>
                ...
                <label class="hero-search__field hero-search__field--select">
                  <div class="hero-search__control hero-search__control--select">
                    <select v-model="heroSearchDisabilityCategory" class="hero-search__select" aria-label="All Disability Types">
                      <option value="">All Disability Types</option>
                      <option v-for="option in heroDisabilityOptions" :key="option" :value="option">{{ option }}</option>
                    </select>
                    <span class="hero-search__caret" aria-hidden="true">
                      <svg viewBox="0 0 20 20" fill="none">
                        <path d="M5 7.5L10 12.5L15 7.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
          ...
          >
            <div
              class="landing-hero__logo-shell hero-logo-float relative w-full max-w-[330px] overflow-visible will-change-transform [transform-style:preserve-3d] max-[900px]:max-w-[280px] max-[640px]:max-w-[210px]"
              :style="heroLogoStyle"
            >
              <img
